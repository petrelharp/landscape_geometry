---
title: "Habitat shapes for central Mojave"
author: "Peter Ralph"
date: "`r Sys.Date()`"
---
...
for (k in 1:nrow(clumpmig)) {
    kk <- which( values(habclumps)[habitable] == k )
    clumpmig[k,] <- tapply( colSums( demog$pollen.migration$M[kk,demog$pollen.migration$habitable.inds,drop=FALSE] ), values(habclumps)[pop$habitable], sum )
}
diag(clumpmig) <- NA
...
refcells <- sapply( 1:nrow(clumpmig), function (k) { sample(which(values(habclumps)==k),1) } )
...
clump_cutoff <- function (maxdist,clumpdist,...) {
    adj <- as( ( is.na(clumpdist) | ( clumpdist < maxdist ) ), "ngCMatrix" )
...
clump_cutoff(5000,clumpdist)
clump_cutoff(15000,clumpdist)
clump_cutoff(25000,clumpdist)
clump_cutoff(37000,clumpdist)
```
...
# here are the clusters of communicating elements
clump2.clusters <- unique(apply(clump2.clump,1,which))
...
cluster.cols2 <- sample(rainbow(length(clump2.clusters)))
...
clump_cutoff(0.01,clumpmig2,main="connected by 1/100 migration")
clump_cutoff(0.001,clumpmig2,main="connected by 1/1000 migration")
clump_cutoff(0.0001,clumpmig2,main="connected by 1/10,000 migration")
clump_cutoff(1e-6,clumpmig2,main="connected by 1/1,000,000 migration")
```
