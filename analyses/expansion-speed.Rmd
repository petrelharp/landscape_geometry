...
so the predicted speed is `r floor((demog$pollen.migration$sigma+demog$seed.migration$sigma)/2*sqrt(germination.args$s-1))` meters per generation.
...
ctab <- data.frame(
           speed = c( expected = mean.sigma * sqrt(2*s) ,
                      observed = AA.speed ),
